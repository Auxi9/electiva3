...
(() => {
    ...
    document.onreadystatechange = () => {
        ...
        if (document.readyState === "complete"){
            Ajax.sendGetRequest(API_URL, {}, 'application/json',
                (function(array){ loadTasks(array) }),
                (function(error){ showError(error) }), true)
        }
    };
    ...
    const addTask = (e) => {
        ...
        Ajax.sendPostRequest(API_URL, task, 'application/json',
            (function (task) {
                addTaskToList(task);
                newTaskInput.value = "";
            }), (function(error){ showError(error) }), true, 'application/json');
    ...
    const addOnChangeEvent = (task) => {
        ...
        checkBox.onchange = (e) => {
            ...
            let task = new Task(checkBox.parentElement.nodeValue);
            let listaActual = checkBox.parentNode.parentNode.parentElement.id;
            if (listaActual === "incomplete-tasks"){
                task.status = TASK_STATUS.DONE;
            }
            checkBox.parentNode.parentNode.parentNode.removeChild(checkBox.parentNode.parentNode);
             Ajax.sendPutRequest(API_URL, task, 'application/json',
                (function () {
                    addTaskToList(task);
                }),
                (function(error){ showError(error) }), true, 'application/json');
    ...
    const editTask = (e) => {
        ...
        buttonOK.onclick = () => {
            ...
            Ajax.sendPutRequest(API_URL, currentTask, 'application/json',
                (function () {
    ...
    const removeTaskFromList = (id) => {
        ...
        document.removeChild(document.getElementById('task-${id}'));
    };
    ...
    const removeTask = (e) => {
        ...
        //     un mensaje de error
        //   - La llamada debe ser asíncrona.
        Ajax.sendDeleteRequest(API_URL, id, 'application/json',
            (function () {
                removeTaskFromList(id)
            }),
            (function(error){ showError(error) }), 'application/json')
    };
})();
